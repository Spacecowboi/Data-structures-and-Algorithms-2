...
const forLoopTwoToThe = (arr) => {
  const newArray = [];
  //.pow to target specific number of times to square, then say which paramter we are squaring
  for(let value of arr){ // say "hey, give me the VALUE of the ARRAY in our function"
...
const forEachTwoToThe = (arr) => {
  const copyArray = [];
  //same as challenge 1, just with for each, remove redundant words and just say for each value in the array, do this.
  arr.forEach(value =>{
    copyArray.push(Math.pow(2,value));
  });
  return copyArray;
};
...
const mapTwoToThe = (arr) => {
  ...
  //map builds new array so we don't have to create a variable containing an empty array
};
